...
- `--source <auto|web|cli|oauth|api>` (default: `auto`).
    ...
    - `api`: API-key/token flow when the provider supports it (OpenAI, Claude Admin API, ClinePass, z.ai, Gemini, Alibaba, Copilot, Kilo, Kimi, MiniMax, Ollama, Warp, OpenRouter, ElevenLabs, Deepgram, Synthetic, DeepSeek, DeepInfra, Neuralwatt, ai&, Moonshot, Doubao, Codebuff, Crof, Venice, AWS Bedrock).
