...
struct LongCatProviderTests {
    ...
    @Test
    func `supplemental fuel cancellation remains cancellation`() async throws {
        let transport = ProviderHTTPTransportHandler { request in
            ...
            case "/api/v1/user-current":
                body = #"{"code":0,"data":{"name":"Leo"}}"#
            case "/api/lc-platform/v1/tokenUsage":
                body = #"{"code":0,"data":{"usage":{"totalToken":500000,"usedToken":120000}}}"#
            case "/api/lc-platform/v1/pending-fuel-packages":
                throw CancellationError()
            default:
                Issue.record("Unexpected LongCat endpoint: \(url.path)")
                body = ""
            }
