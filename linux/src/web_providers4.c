...
CodexBarProvider *codexbar_commandcode_parse(const char *credits,
    ...
    json_object *subscription_data = subscription_root ? member(subscription_root, "data") : NULL;
    ...
    if (total > 0) {
        ...
        add_window(provider, "primary", "Monthly credits", MAX(0, total - remaining), total,
                   reset_ms, has_reset, detail);
        g_free(detail);
    } else if (remaining > 0) {
        add_window(provider, "primary", "Monthly credits", 0, remaining, reset_ms, has_reset, NULL);
    }
