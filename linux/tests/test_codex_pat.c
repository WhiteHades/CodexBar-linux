...
static CodexBarHttpResponse *pat_transport(const CodexBarHttpRequest *request, GError **error) {
    ...
    if (cancel_after_response) g_cancellable_cancel(expected_cancellable);
    return response;
}
...
static void test_malformed_auth_is_unavailable_in_auto(PatFixture *fixture, gconstpointer user_data) {
    (void)user_data;
    char *home = ambient_home(fixture);
...
static void test_pat_cancellation(PatFixture *fixture, gconstpointer user_data) {
    (void)user_data;
    char *home = ambient_home(fixture);
    write_auth(home, "{\"personal_access_token\":\"pat-token\"}");
    g_free(home);
    ...
    GError *error = NULL;
    CodexBarProvider *provider = codexbar_codex_fetch_with_adapters(
        &fixture->config, "api", pat_transport, fake_cli, cancellable, &error);
    g_assert_null(provider);
    ...
    expect_requests(requests, G_N_ELEMENTS(requests));
    expected_cancellable = cancellable;
...
int main(int argc, char **argv) {
    ...
    g_test_add("/codex/pat/malformed-auth-auto-fallback", PatFixture, NULL, fixture_setup, test_malformed_auth_is_unavailable_in_auto, fixture_teardown);
    g_test_add("/codex/pat/cancellation", PatFixture, NULL, fixture_setup, test_pat_cancellation, fixture_teardown);
